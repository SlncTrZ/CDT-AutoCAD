...
- G3 provides one immutable predecessor checkpoint across yielded native chunks, independent final compact-state verification and exact R2 predecessor restoration for logical failure or unknown completion; D15 additionally binds that checkpoint to the originating canonical request UUID: direct in-memory receipts return the raw owner only to that request, durable schema-v2 manifests and persisted semantic/logical journals retain only `sha256(owner_request_id)`, `bridge.recovery.list` exposes only the same one-way `owner_request_fp`, and resolve/finalize/logical-chunk recovery requires the caller to present the original raw owner so foreign adoption fails before CAD mutation;
...
- the current source identity is `0.4.0rc3 / autocad-generic-v1-rc3 / 87 tools` with bridge candidate `0.8.6-d18`; `native_document_identity_initialize` provides an explicit read-back-verified document-lineage bootstrap only for a new empty current space: D18 verifies the bound document remains active before PID side effect, keeps provisional PID write/readback/semantic extraction inside one native transaction, and commits only after verification; legacy/non-empty adoption remains explicit future scope; the five public native strong-integrity write entrypoints still require caller-supplied `document_pid` + `expected_parent_fp` from the state actually planned against; bounded native create includes TEXT/MTEXT/aligned/linear dimensions with optional layer/color assignment in addition to the earlier primitive families;
...
The protocol below remains both implemented contract and normative guardrail. Current public status authority is `docs/CURRENT_CHECKPOINT.md`; accepted live evidence authority is `docs/LIVE_ACCEPTANCE.md`. Maintainer-local notes and raw machine manifests do not override either public authority.
...
- Must restore the predecessor semantic/identity state without requiring UI commands.
- For D18 document-PID bootstrap, PID write/readback/semantic verification share one transaction; abort restores the predecessor PID/lineage and entity state. AutoCAD may still mark the document modified (`DBMOD`) after an aborted bootstrap transaction, so persisted-clean state is a separate D17 invariant rather than part of the D18 identity-rollback claim.
