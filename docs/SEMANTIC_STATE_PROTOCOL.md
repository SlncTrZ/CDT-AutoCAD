...
- MP-G05 has begun with native read-only `3DSOLID` semantics: provider PID carriage is reused, solid geometry participates in deterministic geometry/document fingerprints through a versioned `mass-properties-v1` signature (centroid, volume, extents and inertia terms), and AutoCAD 2027 Session-1 acceptance proved PID/fingerprint stability across save/reopen plus fingerprint sensitivity/restoration across a reversible move. Solid mutation/checkpoint/exact-recovery is still OPEN, and `topology_verified=false` is explicit because the current signature is not a B-rep topology oracle.
