...
| T10 | Dependency/build/DLL drift | Test evidence attached to different executable/runtime | Platform PEP 751 locks, source-tree hash, Git/diff identity, DLL hash and target runtime in manifests | Clean locked install + manifest/evidence comparison; current `0.8.2-mp7` candidate and loaded Session-1 DLL hash are bound in the accepted-artifact registry | Release owner |
