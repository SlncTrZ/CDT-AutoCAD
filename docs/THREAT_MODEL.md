...
> Baseline: MP0-T00 · Version: 3 · Updated: 2026-09-19 +07:00
...
9. A recovery checkpoint owner is a private capability: recovery discovery may expose only a one-way correlation fingerprint, while durable new manifests, journals and retained evidence must not persist or publish the raw owner UUID.
...
| B2 | Provider → filesystem | TOCTOU, path escape and artifact durability boundary. The configured allowed-root identity/ACL is a trusted administrative anchor; descendants beneath that root may be concurrently renamed or replaced by an attacker. |
...
| T02 | Path traversal, symlink/junction/UNC or concurrent descendant namespace swap | Read/write outside authorized roots or to the wrong descendant | Provider-owned I/O must bind actual reads/writes/atomic replace to held directory descriptors/handles, reject redirecting components, and verify bound entry identity; AutoCAD COM APIs that accept pathname strings only remain a bounded lane with pre/post path/state verification and must not be described as race-free | Actual-I/O adversarial swap fixtures on Linux + Windows; full cross-platform regression; historical/live AutoCAD provenance fixtures for pathname-only operations | Provider maintainer |
...
| T10 | Dependency/build/DLL drift | Test evidence attached to different executable/runtime | Platform PEP 751 locks, source-tree hash, Git/diff identity, DLL hash and target runtime in manifests | RC2 exact-lock evidence remains historical; current D18 `0.8.6-d18` Session-1 reload binds deployed bridge SHA-256 `c058c0266664490e4fc05431320aab9fca11dcf6f0d8796241cd1823447f44ae`; a published RC3 release still requires final exact-source release-provenance certification | Release owner |
| T11 | Hot reload activates new worker before old writer is fenced | Mixed-generation mutation/race | Single authoritative generation; stable supervisor request lease; fence new requests; drain old in-flight requests; old-safe + candidate-health gates; pending transaction/recovery/timeout uncertainty blocks activation; failed candidate preserves last healthy generation; protocol/contract-hash/tool-count/policy mismatch blocks promotion | **MP-2 CLOSED / LIVE PASS:** original process acceptance plus the 2026-09-12 then-current `86 tools / 0.8.2-mp7` isolated rerun with `20` successful reloads + `10` injected startup failures, zero pending recovery and verified Session-1 task/process cleanup; this historical evidence is not relabeled as RC3 | Runtime owner |
...
| T15 | Concurrent or reconnecting caller discovers another request's recovery checkpoint and reuses its owner token | Foreign rollback/finalize/chunk adoption or denial of the originating recovery | Generate a canonical per-request owner before dispatch; direct in-memory response returns it only to the originating caller; new checkpoint manifests and persisted journals store only `sha256(owner_request_id)`; `bridge.recovery.list` exposes only that fingerprint; resolve/finalize/logical-chunk paths require the original raw owner and compare its fingerprint to the persisted binding before CAD mutation | D15 focused owner/lost-response negatives + Linux/Windows full regression + C# Release/x64 + AutoCAD 2027 Session-1 lost-response/foreign-adoption fixture on `0.8.5-d15`; zero pending recovery | Semantic/native owner |
| T16 | Active document changes or bootstrap verification fails after document PID creation | Orphan persistent lineage is committed to the wrong/unverified document and later callers treat it as authoritative | Require the runtime-bound document to remain active before PID side effect; perform provisional PID write, PID readback and bounded semantic extraction in one native transaction; re-check active binding before commit; any pre-commit failure aborts PID lineage. The direct-bridge `after_pid_before_verify` fault stage is a fixed enum used only for acceptance and cannot be supplied through the public MCP bootstrap tool | D18 protocol/client negatives + exact-lock Linux/Windows full regression + C# Release/x64 + AutoCAD 2027 Session-1 active-switch and post-PID fault fixture on `0.8.6-d18`; PID returns to null and entity state remains unchanged after abort | Semantic/native owner |
