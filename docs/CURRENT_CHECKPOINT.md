...
- MP-G05 bounded native `3DSOLID` integrity loop is now CLOSED for **planar translate only**: provider PID + `solid-semantic-v2` fingerprinting, `expected_parent_fp` drift refusal, independent persisted read-back, immutable predecessor checkpointing, R0 abort and exact R2 predecessor recovery are live-accepted; rotate/scale, Boolean ACIS mutation and B-rep topology are not implied by this closure, and `topology_verified=false` remains explicit;
...
- **MP-G05 native solid integrity loop:** the initial read slice proved persistent `3DSOLID` PID/fingerprints and save/reopen stability. Live mutation work then exposed that `Solid3d.MassProperties.Extents` is not a translation-faithful geometric bounding box: after `[+2,+1,0]`, its minimum remained `[0,0,0]` while `Solid3d.GeometricExtents` moved correctly. The semantic schema was therefore corrected to `solid-semantic-v2`, using `GeometricExtents` as canonical geometric extents while retaining centroid/volume/inertia mass properties and explicit `topology_verified=false`.
- **MP-G05 Session-1 mutation/recovery acceptance:** one provider-PID `Solid3d` box (volume `480`) translated from centroid `[5,7,3]` to `[7,8,3]` with `COMMITTED_VERIFIED`; a stale predecessor fingerprint was refused as `STATE_DRIFT` without mutation; `after_apply_before_commit` produced exact `R0_ABORT → ROLLED_BACK_VERIFIED`; `after_commit_add_stray` forced `COMMIT_INTEGRITY_FAIL` and exact `R2_CHECKPOINT_RESTORE → ROLLED_BACK_VERIFIED` to the predecessor fingerprint; pending recovery returned to `0`. `rotate_z` remained intentionally refused as `UNSUPPORTED_TARGET_GEOMETRY`.
- **Current regressions after MP-G05 closure:** Linux **386 passed / 6 skipped**; Windows `.171` **385 passed / 7 skipped**; Ruff + compile PASS; C# Release/x64 **0 errors / 3 inherited warning families**. The public identity remains `0.4.0rc1 / autocad-generic-v1-rc1 / 86 tools`.

These results close MP-G05 for the deliberately bounded first native solid mutation family: planar translation under the existing native semantic/checkpoint/recovery chain. They do **not** claim native Boolean recovery, arbitrary 3D transform parity, face/edge topology equivalence or complete ACIS migration; those remain separate future capability decisions rather than hidden implications of this closure.
