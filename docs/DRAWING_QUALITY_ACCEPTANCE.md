...
All reference-driven/user-reviewed execution must also follow `docs/DRAWING_EXECUTION_QA_WORKFLOW.md` and `docs/SEMANTIC_STATE_PROTOCOL.md`. Quality acceptance does not authorize monolithic execution. The current architecture requires each dependent semantic step to continue only from verified state using the strongest accepted route for that family. **N4–N7 + O1 provide the native semantic/mutation/recovery loop for their bounded accepted families; N7 includes checkpoint-backed post-commit R1/R2 recovery including activation-safe R2 after restart, and later G1/G2/G3/MP-G05 scopes add only their explicitly documented guarantees.** Workflows must not project native guarantees onto broader COM/headless routes beyond `docs/CURRENT_CHECKPOINT.md`. Major gates compare structured state against the SourceSemanticModel. Vision is supplemental, not the geometry oracle.
