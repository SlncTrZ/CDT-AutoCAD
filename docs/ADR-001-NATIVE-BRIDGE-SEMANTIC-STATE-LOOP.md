...
This ADR defines the **target architecture** and records a decision that predates implementation. Current status authority is `docs/CURRENT_CHECKPOINT.md`. As of the 2026-09-11 checkpoint, N0–N7 and O1 are closed/live-verified for their bounded scopes; the staged bridge has typed LINE/CIRCLE/ARC/simple-LWPOLYLINE mutation plus checkpoint-backed R0/R1/R2 recovery, including activation-safe R2 after real AutoCAD restart. MP-2 Python MCP hot reload is also **CLOSED / LIVE PASS** with a stable authenticated supervisor, replaceable stateless workers, fail-safe generation promotion/fallback and Windows AutoCAD 2027 Session 1 acceptance. The public MCP provider remains the unchanged 50-tool COM/ezdxf release-candidate baseline; MP-2 closes the runtime prerequisite but does not itself promote the native bridge or change the public tool contract. Canonical MP-2 evidence: `docs/evidence/mp2-hot-reload-2026-09-11.json`.
